...
from dataset import sampler_cub_car
from dataset import sampler_sop
...
parser = argparse.ArgumentParser(description='HSE: Hybrid Species Embedding for Deep Metric Learning'  )
# export directory, training and val datasets, test datasets
parser.add_argument('--LOG_DIR', default='../logs',help = 'Log folder')
parser.add_argument('--dataset', default='cars',help = 'cub, cars,sop')
...
parser.add_argument('--batch-size', default = 120, type = int,dest = 'sz_batch',help = 'Number of samples per batch.')
...
parser.add_argument('--model', default = 'bn_inception',help = 'Model for training eg resnet50,bn_inception')
parser.add_argument('--loss', default = 'Proxy_Anchor',help = 'Criterion for training,eg resnet50,bn_inception')
parser.add_argument('--HSEloss', default = 1,type=int,help = '0:no HSloss,1:use HSloss')
...
# parser.add_argument('--MSDA', default = 'Cutmix',help = 'Mixed Sample Data Augmentation')
parser.add_argument('--a', default = '0.1',type = float,help = 'hyper-parameter a')
...
wandb.init(project='11111', notes=LOG_DIR)
wandb.config.update(args)

os.chdir('/media/d/shq/mycode/Proxy-Anchor-CVPR2020/data/')
data_root = os.getcwd()
...
if args.IPC:
        if(args.dataset=='SOP'):
            balanced_sampler = sampler_sop.BalancedSampler(trn_dataset, batch_size=args.sz_batch,loss=args.loss, images_per_class = args.IPC)
        else:
            balanced_sampler = sampler_cub_car.BalancedSampler(trn_dataset, batch_size=args.sz_batch, images_per_class = args.IPC)
...
elif args.loss == 'EPSHN':
    criterion = losses.EPHNLoss().cuda()

criterion_HSloss=losses.HSELoss().cuda()
# Train Parameters
param_groups = [
...
for epoch in range(0, args.nb_epochs):
    ...
    for batch_idx, (x, y) in pbar:
        ...
        if(args.HSEloss):
            if(args.dataset!='SOP'):
                ...
                x[bz-IPC:bz,:,:, :]=x[0:IPC,:,:, :] 
                x[bz-IPC:bz,:,0:224, 0:112]=x[IPC:2*IPC,:,0:224, 0:112] 
                # elif args.MSDA == 'Mixup':
                #     lam = np.random.beta(60, 60)
                #     x[bz-IPC:bz,:, :,:]=lam*x[0:IPC,:, :, :]+(1-lam)*x[IPC:2*IPC,:,:,:]
                m = model(x.squeeze().cuda())
                loss_aut=criterion(m[:bz-IPC,:], y[:bz-IPC].squeeze().cuda())
                loss_HS=criterion_HSloss(m,y.squeeze().cuda(),IPC,bz,a,args.dataset,args.loss)
                loss= loss_HS+loss_aut
            else:
                x=torch.cat((x,x[0:60,:, :, :]),dim=0)
                y=torch.cat((y,y[0:60]),dim=0)
                print(y)
                for i in range(12):
                    x[120+i*5:120+(i+1)*5,:,:,:]=x[i*10:i*10+5,:,:,:]
                    x[120+i*5:120+(i+1)*5,:,112:224,0:224]=x[i*10+5:10*(i+1),:,112:224,0:224]

                vutils.save_image(x, 'test.jpg')
                m = model(x.squeeze().cuda())
                loss_aut=criterion(m[:120,:], y[:120].cuda())
                loss_our=criterion_HSloss(m, y.squeeze().cuda(),IPC,bz,a,args.dataset,args.loss)
                loss=loss_aut+loss_our
    ...
    if(epoch >= 0):
        ...
        if best_recall[0] < Recalls[0]:
            #NMI
            # with torch.no_grad(): 
            #     pbar = tqdm(enumerate(dl_ev),total=len(dl_ev))
            #     model.eval()
            #     testdata = torch.Tensor()
            #     testlabel = torch.LongTensor()
            #     for batch_idx, (m, y) in pbar:
            #         x=model(m)
            #         testdata = torch.cat((testdata, x.cpu()), 0)
            #         testlabel = torch.cat((testlabel, y))
            #     features = testdata.numpy()
            #     labels = testlabel.numpy()
            #     classN = np.max(labels) + 1
            #     kmeans = KMeans(n_clusters=classN).fit(features)
            #     nmi = normalized_mutual_info_score(labels, kmeans.labels_, average_method='arithmetic')
            #     wandb.log({"NMI": nmi}, step=epoch)
            best_recall = Recalls
            best_epoch = epoch
            if not os.path.exists('{}'.format(LOG_DIR)):
                os.makedirs('{}'.format(LOG_DIR))
            torch.save({'model_state_dict':model.state_dict()}, '{}/{}_{}_best_test.pth'.format(LOG_DIR, args.dataset, args.model))
